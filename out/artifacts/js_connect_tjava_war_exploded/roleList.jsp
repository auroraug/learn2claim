...
<form method="post" id="queryForm" action="<%=path%>/_roleservlet">
...
<script>
    ...
    function addRole() {
        window.location.href = "<%=path%>/_roleservlet?type=2";
    }

    function editRole(roleId) {
        window.location.href = "<%=path%>/_roleservlet?type=2&roleId="+roleId;
    }

    function deleteRole(roleId) {
        if (window.confirm("您确定要删除此条记录吗？")) {
            window.location.href = "<%=path%>/_roleservlet?type=3&roleId="+roleId;
        }
    }
</script>
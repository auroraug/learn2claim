    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            ...
            preparedStatement = connection.prepareStatement("update examrecord set" +
                    " total_score=?,obtain_score=? where address=? and exam_description_id=?");
            ...
            if (preparedStatement.executeUpdate()==1){  //插入成功，带成功提示参数重定向
                resp.sendRedirect("../scholarPage/myExam.jsp?message=success");
            }
            else {  //插入失败，带失败提示参数重定向
                resp.sendRedirect("../scholarPage/myExam.jsp?message=fail");
            }
